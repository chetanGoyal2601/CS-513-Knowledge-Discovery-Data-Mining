...
nbclf = GaussianNB().fit(X_train, np.ravel(y_train,order='C'))
...
for i, row in df_confusion_matrix.iterrows():
    df_confusion_matrix.at[i, 'Predicted Class'] = nbclf.predict([[row['F1'], row['F2'], row['F3'], row['F4'], row['F5'], row['F6'], row['F7'], row['F8'], row['F9']]])[0]
